...
const init = async () => {
  ...
  const coinManager: CoinManagerSingleton = CoinManagerSingleton.getInstance([
    turbos,
    cetus,
    aftermath,
    flowx,
  ...
  const exportPrivateKey = async function (
  ...
  const withdraw = async function (
    ...
    const availableBalance = await walletManager.getSuiBalance(
      ctx.session.publicKey,
    );
    await ctx.reply(
      `Reply with the amount you wish to buy (0 - ${availableBalance} SUI, Example: 0.1):`,
