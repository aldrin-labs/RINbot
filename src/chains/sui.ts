...
SUI_PROVIDER_URL = "https://sui-mainnet.blockvision.org/v1/2bYB32LD0S06isRTaxATLv1mTt6";
...
async initializeSDKInstances() {
  try {
    this.turbos = await TurbosSingleton.getInstance({ suiProviderUrl: this.SUI_PROVIDER_URL, cacheOptions: this.cacheOptions });
    this.cetus = await CetusSingleton.getInstance({
      suiProviderUrl: this.SUI_PROVIDER_URL,
      sdkOptions: clmmMainnet,
      cacheOptions: this. cacheOptions,
    });
    this.aftermath = await AftermathSingleton.getInstance({ cacheOptions: this.cacheOptions });
    this.flowx = await FlowxSingleton.getInstance({ cacheOptions: this.cacheOptions });
    
    this.coinManager = CoinManagerSingleton.getInstance([this.turbos, this.cetus, this.aftermath, this.flowx]);
    this.walletManager = WalletManagerSingleton.getInstance(this.provider, this.coinManager);
    this.providers = [this.turbos, this.cetus, this.aftermath, this.flowx];
    this.routerManager = RouteManager.getInstance(this.providers, this.coinManager);
  } catch (error) {
    console.log(`Could not initialize sdk: ${error}`);
