...
export async function assets(ctx: BotContext): Promise<void> {
  try {
    ...
    const assetsString = allCoinsAssets?.reduce((acc, el) => {
      ...
      const balance = isCoinAssetDataExtended(el) && el.price !== null ? `<b>${el.balance} ${el.symbol?.toUpperCase()} / ${calculate(el.balance, el.price)} USD</b>` : `<b>${el.balance} ${el.symbol}</b>`
