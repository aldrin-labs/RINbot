...
export async function welcomeBonusConversation(conversation: MyConversation, ctx: BotContext): Promise<void> {
  ...
  const caption =
    "To kickstart your journey with us, we're bestowing upon you " +
    `${conversation.session.welcomeBonus.amount} SUI to jumpstart your expedition with us. 🚀 ` +
    "\n\n🤔 👀 Here's the twist: After any withdrawal, your balance must remain at least " +
    `${conversation.session.welcomeBonus.amount} SUI, and the option to export your private key becomes ` +
    `available only after engaging in at least ${WELCOME_BONUS_MIN_TRADES_LIMIT} trades. But fear not, you're ` +
    "all set to trade, reap profits, and withdraw your gains!\n Are you ready to take on the challenge? \n\n🌐 " +
